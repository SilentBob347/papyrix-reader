...
int main() {
  ...
  r.expectTrue(ui::settingsListHitTest({438, 430}, 800, 480, 40, 12).type ==
                   ui::SettingsListHit::Type::Previous,
  ...
  r.expectTrue(network.type == ui::WifiListView::Hit::Type::Row && network.index == 10,
               "Wi-Fi row maps page index");
  wifi.scanning = true;
  ...
  r.expectTrue(confirm.hitTest({dialog.choices[0].x, dialog.choices[0].y}, dialog, 800, 480) == ui::ConfirmView::Hit::Yes,
               "utility confirmation yes");
  r.expectTrue(confirm.hitTest({dialog.choices[1].x, dialog.choices[1].y}, dialog, 800, 480) == ui::ConfirmView::Hit::No,
  ...
  r.expectTrue(calibre.hitTest({75, 430}, 800, 480) == ui::CalibreView::Hit::Back,
               "Calibre back action");
  r.expectTrue(calibre.hitTest({256, 430}, 800, 480) == ui::CalibreView::Hit::Restart,
               "Calibre restart action");
